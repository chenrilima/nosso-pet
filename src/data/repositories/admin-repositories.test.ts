...
describe("admin repositories", () => {
  ...
  it("lists inactive categories and maps group dependency count", async () => { const fake = clientWith({ data: [{ id: "1", name: "A", slug: "a", sort_order: 0, is_active: false, product_option_groups: [{ count: 3 }] }], error: null }); await expect(listAdminCategories(fake.client)).resolves.toMatchObject([{ is_active: false, dependency_count: 3 }]); expect(fake.calls.some((call) => call[0] === "eq" && call[1] === "is_active")).toBe(false); });
  it("creates, updates and toggles categories", async () => { const fake = clientWith(); const values = { name: "A", slug: "a", description: "", sort_order: 0, is_active: true }; await createCategory(fake.client, values); await updateCategory(fake.client, "id", values); await toggleCategory(fake.client, "id", false); expect(fake.calls).toContainEqual(["insert", values]); expect(fake.calls).toContainEqual(["update", values]); expect(fake.calls).toContainEqual(["update", { is_active: false }]); });
  it("blocks deletion when option groups exist", async () => { const fake = clientWith({ data: null, error: null, count: 1 }); await expect(deleteCategory(fake.client, "id")).resolves.toBe("in_use"); expect(fake.calls.some((call) => call[0] === "delete")).toBe(false); });
  it("deletes a category without groups", async () => { const fake = clientWith(); await expect(deleteCategory(fake.client, "id")).resolves.toBe("deleted"); expect(fake.calls.some((call) => call[0] === "delete")).toBe(true); });
});
