...
describe("admin repositories", () => {
  ...
  it("lists inactive categories and maps dependency count", async () => { const fake = clientWith({ data: [{ id: "1", name: "A", slug: "a", sort_order: 0, is_active: false, products: [{ count: 2 }], product_option_groups: [{ count: 3 }] }], error: null }); await expect(listAdminCategories(fake.client)).resolves.toMatchObject([{ is_active: false, dependency_count: 5 }]); expect(fake.calls.some((call) => call[0] === "eq" && call[1] === "is_active")).toBe(false); });
