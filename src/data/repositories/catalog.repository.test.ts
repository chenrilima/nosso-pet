...
function clientWith(results: unknown[], selections: string[] = []) { let index = 0; return { from: () => { const result = results[index++]; const builder = new Proxy({}, { get: (_target, key) => key === "then" ? (resolve: (value: unknown) => unknown) => Promise.resolve(resolve(result)) : key === "select" ? (columns: string) => { selections.push(columns); return builder; } : () => builder }); return builder; } } as unknown as DatabaseClient; }
