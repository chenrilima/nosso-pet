...
export type AdminCategory = Pick<CategoryRow, "id" | "name" | "slug" | "sort_order" | "is_active"> & { product_count: number };
...
export async function listAdminCategories(client: DatabaseClient): Promise<AdminCategory[]> {
  const { data, error } = await client.from("categories").select("id, name, slug, sort_order, is_active, products(count)").order("sort_order").order("name");
  if (error) throw repositoryError("categories", error);
  return data.map((row) => ({ ...row, product_count: row.products[0]?.count ?? 0 }));
}

export async function createCategory(client: DatabaseClient, values: { name: string; slug: string; sort_order: number; is_active: boolean }): Promise<void> {
  const { error } = await client.from("categories").insert(values);
  if (error) throw repositoryWriteError("categories", error);
}

export async function updateCategory(client: DatabaseClient, id: string, values: { name: string; slug: string; sort_order: number; is_active: boolean }): Promise<void> {
