...
export async function updateGalleryStoragePath(client: DatabaseClient, id: string, storagePath: string): Promise<void> { const { error } = await client.from("gallery_images").update({ storage_path: storagePath, position_x: 50, position_y: 50 }).eq("id", id); if (error) throw repositoryWriteError("gallery_images", error); }
export async function updateGalleryImagePosition(client: DatabaseClient, id: string, x: number, y: number): Promise<void> { const { error } = await client.from("gallery_images").update({ position_x: x, position_y: y }).eq("id", id); if (error) throw repositoryWriteError("gallery_images", error); }
