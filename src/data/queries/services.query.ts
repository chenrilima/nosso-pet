...
async function getServices(context: PublicQueryContext | undefined, bookable: boolean) {
  ...
  return runPublicQuery("services", async () => (await (bookable ? listBookableServiceRows(ctx.client) : listActiveServiceRows(ctx.client))).map((row) => toService(row, ctx.assetUrl)));
}
