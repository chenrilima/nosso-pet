...
describe("public query result", () => {
  ...
  it("converts repository errors without leaking their message", async () => {
    const result = await runPublicQuery("catalog", async () => { throw new RepositoryError("catalog", "read", "08006"); });
    expect(result).toEqual({ ok: false, error: { code: "DATA_ACCESS_ERROR", entity: "catalog", operation: "read", retryable: true } });
