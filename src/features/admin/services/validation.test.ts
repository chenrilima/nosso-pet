...
function form(overrides: Record<string, string> = {}) { const data = new FormData(); Object.entries({ name: "Banho", iconKey: "bath", pricingType: "quote", price: "", priceFrom: "", durationMinutes: "", sortOrder: "0", ...overrides }).forEach(([key, value]) => data.set(key, value)); return data; }
describe("service validation", () => {
  ...
  it.each(["-1", "1.5", "10081"])("rejects invalid duration %s", (durationMinutes) => expect(validateService(form({ durationMinutes })).fieldErrors.durationMinutes).toBeTruthy());
  ...
  it("supports status, bookable, featured and ordering", () => { const data = form({ sortOrder: "4" }); for (const key of ["isActive", "isBookable", "isFeatured"]) data.set(key, "on"); expect(validateService(data).values).toMatchObject({ is_active: true, is_bookable: true, is_featured: true, sort_order: 4 }); });
});
