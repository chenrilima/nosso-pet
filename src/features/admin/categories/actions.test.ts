...
const mocks = vi.hoisted(() => ({ requireAdmin: vi.fn(), revalidatePath: vi.fn(), createCategory: vi.fn(), updateCategory: vi.fn(), toggleCategory: vi.fn(), deleteCategory: vi.fn(), createClient: vi.fn(() => Promise.resolve({})) }));
vi.mock("next/cache", () => ({ revalidatePath: mocks.revalidatePath }));
vi.mock("@/features/admin/auth/server", () => ({ requireAdmin: mocks.requireAdmin }));
vi.mock("@/lib/supabase/server", () => ({ createClient: mocks.createClient }));
vi.mock("@/data/repositories/categories.repository", () => ({ createCategory: mocks.createCategory, updateCategory: mocks.updateCategory, toggleCategory: mocks.toggleCategory, deleteCategory: mocks.deleteCategory }));
...
describe("category actions", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.requireAdmin.mockResolvedValue({ id: "admin" }); mocks.deleteCategory.mockResolvedValue("deleted"); });
  it("authorizes, creates and revalidates the home", async () => { await expect(createCategoryAction(previous, form())).resolves.toMatchObject({ ok: true }); expect(mocks.requireAdmin).toHaveBeenCalledOnce(); expect(mocks.createCategory).toHaveBeenCalledWith({}, { name: "Rações", slug: "racoes", sort_order: 1, is_active: true }); expect(mocks.revalidatePath).toHaveBeenCalledWith("/"); });
  ...
  it("blocks deletion in use and does not revalidate", async () => { mocks.deleteCategory.mockResolvedValue("in_use"); await expect(deleteCategoryAction(id, previous)).resolves.toMatchObject({ ok: false, message: expect.stringContaining("produtos vinculados") }); expect(mocks.revalidatePath).not.toHaveBeenCalled(); });
