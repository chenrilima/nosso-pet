...
describe("category validation", () => {
  ...
  it("accepts a valid create and status", () => { const data = new FormData(); data.set("name", "Rações"); data.set("sortOrder", "2"); data.set("isActive", "on"); expect(validateCategory(data).values).toEqual({ name: "Rações", slug: "racoes", sort_order: 2, is_active: true }); });
