export type CategoryValues = { name: string; slug: string; description: string; sort_order: number; is_active: boolean };
...
export function validateCategory(data: FormData): { values?: CategoryValues; fieldErrors: Record<string, string> } {
  ...
  if (description.length > 300) fieldErrors.description = "Use no máximo 300 caracteres.";
  ...
  return Object.keys(fieldErrors).length ? { fieldErrors } : { fieldErrors, values: { name, slug, description, sort_order: sortOrder, is_active: data.get("isActive") === "on" } };
}
