...
function CategoryActions({ category }: { category: AdminCategory }) {
  ...
  return <div className="space-y-3"><div className="flex flex-wrap gap-2"><form action={toggleAction}><button className="btn btn-secondary min-h-11" disabled={togglePending}>{togglePending ? "Alterando…" : category.is_active ? "Desativar" : "Ativar"}</button></form>{confirming ? <><form action={deleteAction}><button className="btn min-h-11 bg-red-700 text-white" disabled={deletePending}>{deletePending ? "Excluindo…" : "Confirmar exclusão"}</button></form><button type="button" className="btn btn-secondary min-h-11" onClick={() => setConfirming(false)}>Cancelar</button></> : <button type="button" className="btn min-h-11 border border-red-200 bg-white text-red-700" onClick={() => setConfirming(true)} disabled={category.product_count > 0} title={category.product_count > 0 ? "Há produtos vinculados" : undefined}>Excluir</button>}</div><FormFeedback state={toggleState.message ? toggleState : deleteState} />{confirming && <p className="text-sm font-bold text-red-700">Excluir remove permanentemente. Esta ação não pode ser desfeita.</p>}</div>;
}
...
export function CategoryCard({ category }: { category: AdminCategory }) {
  ...
  return <article className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-xl font-black text-olive">{category.name}</h2><p className="text-sm font-bold text-stone-500">{category.product_count} produto(s) vinculado(s)</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${category.is_active ? "bg-green-100 text-green-800" : "bg-stone-200 text-stone-700"}`}>{category.is_active ? "Ativa" : "Inativa"}</span></div><form action={action} className="space-y-4"><CategoryFields category={category} state={state} /><FormFeedback state={state} /><button className="btn btn-primary w-full sm:w-auto" disabled={pending}>{pending ? "Salvando…" : "Salvar edição"}</button></form><div className="my-4 h-px bg-stone-200" /><p className="mb-3 text-sm font-semibold text-stone-600">Desativar oculta do site. Excluir remove permanentemente e só é permitido sem produtos.</p><CategoryActions category={category} /></article>;
}
