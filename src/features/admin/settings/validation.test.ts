...
function validForm() {
  ...
  Object.entries({ name: "Nosso Pet", shortName: "Nosso Pet", phone: "(11) 4558-3210", whatsapp: "(11) 96644-2719", instagramHandle: "@nossopet_taboao", instagramUrl: "https://instagram.com/nossopet_taboao", addressLine: "Rua 1", district: "Centro", city: "Taboão", state: "SP", postalCode: "06765-000", mapsUrl: "https://www.google.com/maps/search/?api=1&query=Nosso+Pet", mapsEmbedUrl: "https://www.google.com/maps?q=Nosso+Pet&output=embed" }).forEach(([key, value]) => data.set(key, value));
...
describe("business settings validation", () => {
  ...
  it("requires HTTPS and allowlisted public hosts", () => {
    const http = validForm(); http.set("instagramUrl", "http://instagram.com/pet"); http.set("mapsUrl", "http://www.google.com/maps");
    expect(validateBusinessSettings(http).fieldErrors).toMatchObject({ instagramUrl: expect.any(String), mapsUrl: expect.any(String) });
    const arbitraryEmbed = validForm(); arbitraryEmbed.set("mapsEmbedUrl", "https://example.com/maps/embed");
    expect(validateBusinessSettings(arbitraryEmbed).fieldErrors.mapsEmbedUrl).toContain("Google Maps");
  });
