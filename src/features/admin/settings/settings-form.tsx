...
function Input({ label, name, defaultValue, state, type = "text", maxLength = 120 }: { label: string; name: string; defaultValue: string; state: ReturnType<typeof useSettingsState>[0]; type?: string; maxLength?: number }) {
  return <label className="block text-sm font-extrabold text-olive">{label}<input className={fieldClass} name={name} type={type} defaultValue={defaultValue} maxLength={maxLength} required /><FieldError state={state} name={name} /></label>;
}
...
export function SettingsForm({ row }: { row: BusinessSettingsRow }) {
  ...
  return <form action={action} className="space-y-6">
    ...
    <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="mb-4 text-xl font-black text-olive">Identidade e contato</h2><div className="grid gap-4 sm:grid-cols-2">
      ...
      <Input label="Telefone" name="phone" defaultValue={row.phone} state={state} /><Input label="WhatsApp" name="whatsapp" defaultValue={row.whatsapp} state={state} />
    ...
    <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="mb-4 text-xl font-black text-olive">Endereço e mapas</h2><div className="grid gap-4 sm:grid-cols-2">
      ...
      <Input label="CEP" name="postalCode" defaultValue={row.postal_code} state={state} /><div />
    ...
    <section className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="text-xl font-black text-olive">Horários</h2><p className="mb-4 mt-1 text-sm font-semibold text-stone-600">Marque “Aberto” e informe o intervalo. Dias desmarcados são salvos como fechados.</p><div className="space-y-3">{BUSINESS_DAYS.map(([day, label]) => { const value = parseHours(row.hours, day); return <fieldset key={day} className="grid gap-3 rounded-xl bg-stone-50 p-3 sm:grid-cols-[1fr_auto_auto] sm:items-end"><legend className="sr-only">{label}</legend><label className="flex min-h-11 items-center gap-2 font-extrabold text-olive"><input type="checkbox" name={`hours_${day}_open`} defaultChecked={value.open} />Aberto — {label}</label><label className="text-sm font-bold">De<input className={fieldClass} type="time" name={`hours_${day}_start`} defaultValue={value.start} /></label><label className="text-sm font-bold">Até<input className={fieldClass} type="time" name={`hours_${day}_end`} defaultValue={value.end} /></label><div className="sm:col-span-3"><FieldError state={state} name={`hours_${day}`} /></div></fieldset>; })}</div></section>
