...
describe("admin dashboard repository", () => {
  it("uses exact head-only counts and returns the summary", async () => {
    const counts: Record<string, number> = { product_options: 12, categories: 4, services: 8, faqs: 7, gallery_images: 0 };
    const calls: unknown[][] = [];
    ...
    await expect(getAdminDashboardSummary(client)).resolves.toEqual({ productOptions: 12, categories: 4, services: 8, faqs: 7, galleryImages: 0 });
