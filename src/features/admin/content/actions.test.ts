...
const mocks = vi.hoisted(() => ({ requireAdmin: vi.fn(), updateHomeContent: vi.fn(), createClient: vi.fn(() => Promise.resolve({})), revalidatePath: vi.fn() }));
vi.mock("next/cache", () => ({ revalidatePath: mocks.revalidatePath }));
vi.mock("@/features/admin/auth/server", () => ({ requireAdmin: mocks.requireAdmin }));
vi.mock("@/lib/supabase/server", () => ({ createClient: mocks.createClient }));
vi.mock("@/data/repositories/business.repository", () => ({ updateHomeContent: mocks.updateHomeContent }));
