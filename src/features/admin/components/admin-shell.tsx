...
function Brand() {
  return <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-brand text-white"><PawPrint size={22} aria-hidden="true" /></span><div><p className="font-black leading-tight text-olive">Nosso Pet</p><p className="text-xs font-bold text-stone-500">Administração</p></div></div>;
}
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f1]">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-72 flex-col border-r border-stone-200 bg-white p-5 md:flex">
        <Brand /><div className="my-6 h-px bg-stone-200" /><AdminNav /><div className="mt-auto border-t border-stone-200 pt-4"><p className="mb-3 truncate px-3 text-xs font-bold text-stone-500" title={admin.email}>{admin.displayName || admin.email}</p><LogoutButton /></div>
      </aside>
