...
async function verifyUpload(client: DatabaseClient, path: string, directory: "gallery" | "products" | "hero", requirement: string) {
...
export async function verifyHeroUpload(client: DatabaseClient, path: string): Promise<void> {
  ...
  await verifyUpload(client, path, "hero", "O arquivo enviado não atende aos requisitos da imagem principal.");
}
