...
As imagens têm vínculos explícitos. Um upload em `/admin/gallery` cria ou substitui `gallery_images.storage_path` e aparece somente na seção Galeria. Um upload no editor de um produto atualiza `products.image_path` e aparece somente no card desse produto; usa a mesma otimização da Galeria e um UUID em `products/`, com troca segura (grava o path novo antes de remover o arquivo anterior) e remoção independente do produto. `services.image_path` permanece um campo preparado para evolução futura: os cards atuais foram desenhados em torno de `icon_key`. O Hero continua sendo o asset estático `public/images/hero-pets.png`; a Galeria não controla Hero, Produto ou Serviço e não funciona como CMS genérico de imagens.
